...
image box_filter_image(image im, int s)
{
    ...
    for (k = 0; k < im.c; k++)    {
        for (j = 0; j < im.h; j++)        {
            for (i = 0; i < im.w; i++)            {
                tl = get_pixel(integ,i-w-1,j-w-1,k);
                tr = get_pixel(integ,i+w,j-w-1,k);
                bl = get_pixel(integ,i-w-1,j+w,k);
                br = get_pixel(integ,i+w,j+w,k);
                v = (br + tl - tr - bl)/(float)d;
                set_pixel(S,i,j,k,v);
            }
        }
    }
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    for (i = 0; i < im.w*im.h; i++) {
        S.data[i+im.w*im.h*0] = Ix.data[i]*Ix.data[i];
        S.data[i+im.w*im.h*1] = Iy.data[i]*Iy.data[i];
        S.data[i+im.w*im.h*2] = Ix.data[i]*Iy.data[i];
        S.data[i+im.w*im.h*3] = Ix.data[i]*It.data[i];
        S.data[i+im.w*im.h*4] = Iy.data[i]*It.data[i];
    }
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            // assert(det != 0.f);
            // float vx = (1/det)*((-Ixt)*Iyy + (-Ixy)*(-Iyt));
            // float vy = (1/det)*((-Ixt)*(-Ixy) + (Ixx)*(-Iyt));
