...
{
    /* Found the wording to be confusing.
    Slide 69, Lect. 12 shows it nicely. 
    For a partial derivative of the loss function, (intermediate) layers' gradient is 
    merely the derivative of the activation function wrt to its pre-activation sum.
    Writing it out makes the wording smh obvious. Dario
    */
    int i, j;
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            ...
            if(a == LOGISTIC){
                ...
                // TODO
                d.data[i][j] *= (double)(x > 0 ? 1 : 0);
            } else if (a == LRELU){
                // TODO
                d.data[i][j] *= (double)(x > 0 ? 1 : 0.1);
            }
        }
    }
}
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    // save it to l->v
    // matrix dw = axpy_matrix(-decay,l->w,l->dw);
    // dw = axpy_matrix(momentum,l->v,dw);
    // free_matrix(l->v);
    // l->v = dw;
    matrix dw = axpy_matrix(momentum, l->v,
                            axpy_matrix(-decay, l->w, l->dw));
    free_matrix(l->v);
    l->v = dw;

    // Update l->w
    matrix w_t1 = axpy_matrix(rate,dw,l->w);
    free_matrix(l->w);
    l->w = w_t1;
